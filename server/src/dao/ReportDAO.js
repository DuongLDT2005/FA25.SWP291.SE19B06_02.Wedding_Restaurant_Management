...
class ReportDAO {
    ...
    static async createReport(reportData) {
        ...
        const r = await report.create({ userID, restaurantID, reviewID, targetType, reasonType, content, status, seen });
        return toDTO(r);
    }
     ...
     */
    static async list({ status, seen, userID, restaurantID, reviewID, limit, offset } = {}) {
        const where = {};
        ...
        if (restaurantID !== undefined && restaurantID !== null) where.restaurantID = restaurantID;
        if (reviewID !== undefined && reviewID !== null) where.reviewID = reviewID;

        const rows = await report.findAll({ where, order: [['createdAt', 'DESC']], ...(limit ? { limit } : {}), ...(offset ? { offset } : {}) });
        return toDTOs(rows);
    }

    static async getByUserID(userID) {
        const rows = await report.findAll({ where: { userID }, order: [['createdAt', 'DESC']] });
        return toDTOs(rows);
    }

    static async getByRestaurantID(restaurantID) {
        const rows = await report.findAll({ where: { restaurantID }, order: [['createdAt', 'DESC']] });
        return toDTOs(rows);
    }

    static async getByReviewID(reviewID) {
        const rows = await report.findAll({ where: { reviewID }, order: [['createdAt', 'DESC']] });
        return toDTOs(rows);
    }
